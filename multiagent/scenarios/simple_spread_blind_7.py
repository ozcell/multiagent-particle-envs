...
class Scenario(BaseScenario):
    ...
    def observation(self, agent, world):
        ...
        entity_pos = []
        flag = []
        if agent is world.agents[world.leader]:
            flag.append(np.ones(1))
            for entity in world.landmarks:  # world.entities:
	            entity_pos.append(entity.state.p_pos - agent.state.p_pos)
        else: 
            flag.append(np.zeros(1))        
            for entity in agent.state.false_l_pos:  # false entities:
	            entity_pos.append(entity - agent.state.p_pos)
        # entity colors
        entity_color = []
        for entity in world.landmarks:  # world.entities:
